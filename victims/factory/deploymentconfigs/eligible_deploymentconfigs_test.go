...
func TestEligibleDeploymentConfigs(t *testing.T) {
	v1deplepl := newDeploymentConfig(
		NAME,
		map[string]string{
	...
	victims, _ := EligibleDeploymentConfigs(client, NAMESPACE, &metav1.ListOptions{})
...
func TestIsEnrolled(t *testing.T) {
	...
	assert.Equal(t, b, true, "Expected deploymentconfig to be enrolled")
}
...
func TestIsNotEnrolled(t *testing.T) {
	...
	assert.Equal(t, b, false, "Expected deploymentconfig to not be enrolled")
}
