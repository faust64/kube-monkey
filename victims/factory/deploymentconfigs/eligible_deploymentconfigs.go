...
// EligibleDeploymentConfigs gets all eligible deploymentconfigs that opted in (filtered by config.EnabledLabel)
func EligibleDeploymentConfigs(clientset kube.Interface, namespace string, filter *metav1.ListOptions) (eligVictims []victims.Victim, err error) {
	enabledVictims, err := clientset.AppsV1().DeploymentConfigs(namespace).List(*filter)
	if err != nil {
		return nil, err
	}
...
// Checks if the deploymentconfig is currently enrolled in kube-monkey
func (d *DeploymentConfig) IsEnrolled(clientset kube.Interface) (bool, error) {
	deploymentconfig, err := clientset.AppsV1().DeploymentConfigs(d.Namespace()).Get(d.Name(), metav1.GetOptions{})
	if err != nil {
		return false, nil
	}
	return deploymentconfig.Labels[config.EnabledLabelKey] == config.EnabledLabelValue, nil
}

// Returns current killtype config label for update
func (d *DeploymentConfig) KillType(clientset kube.Interface) (string, error) {
	deploymentconfig, err := clientset.AppsV1().DeploymentConfigs(d.Namespace()).Get(d.Name(), metav1.GetOptions{})
	if err != nil {
		return "", err
	}

	killType, ok := deploymentconfig.Labels[config.KillTypeLabelKey]
	if !ok {
...
func (d *DeploymentConfig) KillValue(clientset kube.Interface) (int, error) {
	deploymentconfig, err := clientset.AppsV1().DeploymentConfigs(d.Namespace()).Get(d.Name(), metav1.GetOptions{})
	if err != nil {
		return -1, err
	}

	killMode, ok := deploymentconfig.Labels[config.KillValueLabelKey]
	if !ok {
